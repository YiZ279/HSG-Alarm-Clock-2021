...
## Repository
There are five files distributed within this repository. "README.md" is used for a project description as well as instructions. "HSG_Alarm_Clock.ipynb" and "HSG_Alarm_Clock.py" contain code. The "Screenshot" file shows the interface that pops up when the code is run. The "St. Gallen Bahnhof - Bus Nr. 9" file shows the schedule of the bus number 9. 

In the "HSG_Alarm_Clock.ipynb" file, the following inputs are entered in the user interface as an example: "Going to university" is ticked, time to be at uni is set at 14:00 and the user needs 16min to get ready (current time is 13:31). 
